...
app.MapGet("/api/v1/metadata/module-registration", (IOptions<ServiceOptions> options) =>
    Results.Ok(PluginsPlatformCatalog.BuildModule(options.Value)));

app.MapGet("/api/v1/metadata/permissions", () =>
    Results.Ok(new { items = PluginsPlatformCatalog.BuildPermissions() }));

app.MapGet("/api/v1/plugins/info", (IOptions<ServiceOptions> options) => Results.Ok(new
{
    service = options.Value.Name,
    message = "plugins-service is connected to platform-core and ready for feature implementation.",
}));
