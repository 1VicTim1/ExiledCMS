...
app.MapGet("/api/v1/metadata/module-registration", (IOptions<ServiceOptions> options) =>
    Results.Ok(UsersRolesPlatformCatalog.BuildModule(options.Value)));

app.MapGet("/api/v1/metadata/permissions", () =>
    Results.Ok(new { items = UsersRolesPlatformCatalog.BuildPermissions() }));

app.MapGet("/api/v1/users-roles/info", (IOptions<ServiceOptions> options) => Results.Ok(new
{
    service = options.Value.Name,
    message = "users-roles-service is connected to platform-core and ready for feature implementation.",
}));
